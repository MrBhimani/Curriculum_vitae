...
#        return int((mass + (charge - 1) * spytrometer_proton) / (charge*self.bin_width) + 1.0 - self.bin_offset)
    ...
    def get_peptide_fragmentation(self, peptide_sequence):
        ...
        for mod in re.finditer(self.pattern, peptide_sequence):  # calc modifications' mass
            ...
            if peptide_sequence[location] == "-":
                mod_peptide_aa_mass[-1] += float(mod.group(1))
                break
            if location - offset < len(mod_peptide_aa_mass):
                mod_peptide_aa_mass[location - offset] += float(mod.group(1))
            else:
                print(f"Index out of range: location={location}, offset={offset}, len(mod_peptide_aa_mass)={len(mod_peptide_aa_mass)}")

            # print('mass:', mod_peptide_aa_mass)
            # print('mod_peptide_aa_mass[location - offset]',mod_peptide_aa_mass[location - offset] )
            offset += mod.end() - mod.start()
            # print('offset', offset)
    ...
    def plot_spectrum(self, spectrum_id, show_annotation=True, peptide_seq=None, filename=None, dp_peptide_score=None, Spectrum_Neutral_Mass=None, font_size=24):
        fg=plt.figure(figsize=(20,5))
        fg.patch.set_facecolor('xkcd:white')
        plt.rc('font', size=font_size)          # controls default text sizes
        plt.rc('axes', titlesize=font_size)     # fontsize of the axes title
        ...
        if show_annotation == True and peptide_seq is not None:
            ...
            # plt.title('Seq:{}  Charge:{}   Spectrum Neutral Mass: {} Peptide Neutral Mass: {} DP Peptide Score:{}'.format(peptide_seq, spectrum.charge, spectrum.neutral_mass, peptide_mass,dp_peptide_score ),fontsize=22)
            ...
            # Initialize a counter for the total number of peaks plotted
            total_plotted_peaks = 0
                ...
                for ion_type in peptide_peaks[charge].keys():
                    ...
                    try:
                        ...
                        for mod in modification_list:
                            ...
                            pept_seq[location-offset-1] += modification
                            offset += mod.end()-mod.start()
                    except:
                    ...
                    for i in range(len(AA_pos)):
                        ...
                        if i == 0:
                            ...
                            plt.text(AA_pos[i], max_y+range_y*cnt/10+range_y*0.02, pept_seq[i], 
                                    ...
                                    fontsize=font_size-12)
                    cnt += 1
            # print(f"Total number of peaks plotted: {total_plotted_peaks}")
            if total_plotted_peaks > 10:
                # plt.title('Seq:{}  Scan:{}  Charge:{} spectrum_id:{}  Spectrum Neutral Mass:{} Peptide Neutral Mass:{} DP Peptide Score:{} Total No of Matching Peaks:{}'.format(peptide_seq, spectrum.scan_id,  spectrum.charge, spectrum.spectrum_id,  spectrum.neutral_mass, peptide_mass, dp_peptide_score, total_plotted_peaks), fontsize=22)
                plt.title('Seq:{}  Scan:{}  Charge:{} spectrum_id:{}\nSpectrum Neutral Mass:{} Peptide Neutral Mass:{} DP Peptide Score:{} Total No of Matching Peaks:{}'.format(peptide_seq, spectrum.scan_id,  spectrum.charge, spectrum_id,  Spectrum_Neutral_Mass, peptide_mass, dp_peptide_score, total_plotted_peaks), fontsize=22)
                plt.xlim(min_x, max_x)
                if filename is not None:
                ...
                print("Total number of matching peaks is less than or equal to 10. Not plotting the figure.")
    def plot_intensity_change(filename):
        ...
        print('A_frequency_bins_match', frequency_bins_match)
        ...
        print('frequency_bins_match', frequency_bins_match)

...
class Spytrometer:
    ...
    def load_data(self, path_to_file, min_peak_th=10, data_type='humvar', scan_ids=None):
        ...
        if data_type == 'LUAD':
            msrun = pymzml.run.Reader(path_to_file)
            for pymzml_spectrum in msrun:

                if pymzml_spectrum.ms_level == 2:
                    # Extract scan number from the spectrum id
                    if isinstance(pymzml_spectrum.ID, str):
                        scan_id = int(pymzml_spectrum.ID.split('scan=')[-1])
                    elif isinstance(pymzml_spectrum.ID, int):
                        scan_id = pymzml_spectrum.ID
                    else:
                        raise ValueError("Unexpected spectrum ID type")

                    # Extract charge state and precursor mass
                    charge_state = int(pymzml_spectrum.selected_precursors[0]['charge'])
                    precursor_mass = float(pymzml_spectrum.selected_precursors[0]['mz'])

                    spectrum_record = Spectrum(
                        path_to_file,  # path to file
                        scan_id,  # scan_id
                        np.float_(pymzml_spectrum.mz),  # mz array
                        np.float_(pymzml_spectrum.i),  # intensity array
                        charge_state,  # charge
...
#        return int((mass + (charge - 1) * spytrometer_proton) / (charge*self.bin_width) + 1.0 - self.bin_offset)
    ...
    def plot_spectrum(self, spectrum_id, show_annotation=True, peptide_seq=None, filename=None, dp_peptide_score=None, font_size=24):
        fg=plt.figure(figsize=(20,5))
        fg.patch.set_facecolor('xkcd:white')
        plt.rc('font', size=font_size)          # controls default text sizes
        plt.rc('axes', titlesize=font_size)     # fontsize of the axes title
